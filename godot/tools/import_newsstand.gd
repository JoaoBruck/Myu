...
func _initialize() -> void:
	...
	var height := roundi(94.0*art.get_height()/art.get_width())
	assert(height <= 78)
	art.resize(94,height,Image.INTERPOLATE_NEAREST)
	var cell := Image.create(96,80,false,Image.FORMAT_RGBA8)
	cell.blit_rect(art,Rect2i(Vector2i.ZERO,art.get_size()),Vector2i(1,78-height))
